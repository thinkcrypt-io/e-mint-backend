...
const userShopRegisterController = async (req: any, res: Response): Promise<Response> => {
	...
	try {
		...
		const sub = '67a07986190878bdd49ef2f8';
		...
		const findPackage = await Subscription.findById(sub);
		...
		savedShop.package = savedSubscription._id;
		savedShop.expire = endDate;
		...
		sendMail({
			...
			body: `A new shop has been registered with the name ${shopName} and email ${email}`,
			title: 'Mint Registration',
		});
...
export default userShopRegisterController;
