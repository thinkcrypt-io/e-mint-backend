...
import { active, Settings } from '../../imports.js';
import {
	getString,
	colors,
	getNumber,
...
const schema = new Schema<any>(
	{
		...
		content: {
			...
			sponsoredBannerThree: {
				...
				hide: getBoolean(false),
				css: ImageSchema({
					src: 'https://arewabooks.s3.us-east-2.amazonaws.com/1705388894998804.png_1735449812864',
					width: 'full',
				}),
			},
			...
			productPage: {
				...
				//related product
				relatedProductBg: colors?.bg,
				relatedProductBorder: colors?.fg,
				relatedProductHeading: TextSchema({
					fontSize: {
						base: 14,
						md: 16,
					},
				...
				relatedProductPrice: TextSchema({
					fontSize: {
						base: 14,
						md: 16,
					},
				...
				//product details bg
				productDetailsBg: colors?.bg,

				// product specification bg
				productSpecsBg: colors?.bg,
