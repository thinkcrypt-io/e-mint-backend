...
import {
	...
	getSum,
	InventoryAdjustment,
	inventoryAdjustmentSettings as settings,
	customQuery,
	extendRequestBody,
} from '../imports.js';
import adjustStock from '../controllers/inventory/adjustStock.middleware.js';
...
const damageMiddleware = [customQuery({ query: { reason: 'damage' } })];
const damagePostMiddleware = [extendRequestBody({ query: { reason: 'damage' } })];
...
router.get('/damages/:id', ...getByIdMiddleware, getDocumentById(config.QUERY_OPTIONS));
