...
import { Transfer as Model, transferSettings as settings } from '../models/index.js';
import {
	deleteDocument,
	createDocument,
	updateDocument,
...
const middlewares = {
	...
	getAll: [protect, paginate, filter(config.FILTER_OPTIONS)],
	//Middleware for updating a category
	update: [protect, validate(config.VALIDATORS.UPDATE)],
	...
	count: [protect],
	//Middleware for sum
	sum: [protect, filter(config.FILTER_OPTIONS)],
...
router.get('/get/sum/:field', ...middlewares.sum, getSum(config.MODEL));
